...
if [[ ! -d "$SOURCE_DIR" ]]; then
    ...
    log "El repositorio ya esta clonado en $SOURCE_DIR."
fi

# Cambiar al branch o tag especifico
log "Cambiando a la version $VERSION..."
cd "$SOURCE_DIR"
git fetch --all
...
log "Generando archivos de configuracion con autogen.pl..."
./autogen.pl

# Configurar el entorno de compilación
log "Configurando PMIx para su instalacion en $INSTALL_PREFIX..."
cd "$BUILD_DIR"
...
log "Instalacion completada. PMIx $VERSION esta instalado en $INSTALL_PREFIX."
...
- **Hacer el script ejecutable:**
    ...
    ```bash
    chmod +x install_pmix.sh
    ```
    
- **Ejecutar script usando `sudo`:**
    
    ```bash
    sudo ./install_pmix.sh
    ```
